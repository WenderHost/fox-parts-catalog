        ...
        window.setTimeout(function(){
          if( isVisible('#rootfoxselect') ){
            if( typeof window.FoxSelect !== 'undefined' ){
              ...
              console.warn("\n---- 🚨🚨🚨🚨 ----\n" + 'window.FoxSelect is `undefined`!' + "\nThis is most likely due to `yarn build` placing the app init code in a different chuck than the one we are loading. Check /foxselect/static/index.html for the files required to load FoxSelect." + "\n---- 🚨🚨🚨🚨 ----");
            }
            if( ! isIE() )
