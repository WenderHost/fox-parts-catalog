...
(function($){
  ...
  /**
   * Gets the part and description table.
   *
   * @param      {<type>}  parts   The parts
   * @return     {<type>}  The part and description table.
   */
  function getPartAndDescriptionTable( parts ){
    if( 0 < parts.length ){
      var rows = [];
      for( const part of parts ){
        let partLoad = ( wpvars.partNumber.load )? `/${wpvars.partNumber.load}` : '' ;
        rows.push(`<tr><td><a href="/foxpart/F${wpvars.partNumber.search}/${wpvars.partNumber.frequency}${partLoad}" target="_blank">${part.name}</a></td><td>${part.description}</td></tr>`)
      }
      return table = `<table style="margin-top: 20px;"><thead><tr><th style="width: 10%;">Part</th><th style="width: 90%">Description</th></tr></thead><tbody>${rows.join('')}</tbody></table>`;
    }
  }
