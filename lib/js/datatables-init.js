...
(function($){
  ...
  const partSeriesTable = $('table#partseries').DataTable({
    ...
    'columns': [
      ...
      { 'data': 'part_type' },
      { 'data': function(d){
        let details = '<strong>Package Size:</strong> ' + d.length_mm + 'x' + d.height_mm + 'mm<br><strong>Weight:</strong> ' + d.individual_part_weight_grams + 'g<br><strong>ECCN:</strong> ' + d.export_control_classification_number + '<br><strong>Cage Code:</strong> ' + d.cage_code;
        if( d.part_type && 'oscillator' == d.part_type.toLowerCase() )
          details+= '<br><strong>Output:</strong> ' + d.output;
        return details;
      }
      },
      {
